...
export const updateProfile = async (req: AdminAuthRequest, res: Response) => {
  try {
    ...
    if (file) {
      const uploadResult = await uploadToCloudinary(file.buffer, 'auriq_admin');
...
export const updatePassword = async (req: AdminAuthRequest, res: Response) => {
  try {
    const adminId = req.admin!.id;
...
export const logoutOtherDevices = async (req: AdminAuthRequest, res: Response) => {
  try {
    const adminId = req.admin!.id;
