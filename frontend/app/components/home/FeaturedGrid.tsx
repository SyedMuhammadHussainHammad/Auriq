...
export default function FeaturedGrid({ products = [] }: { products?: any[] }) {
  ...
  return (
    <section className="py-24 bg-perfume-main relative overflow-hidden" id="featured-grid">
      ...
      <div className="container-lux relative z-10">
        <div className="flex justify-between items-end mb-16 border-b border-foreground/5 pb-6">
          <div>
            ...
            <p className="mt-4 text-sm text-foreground/70 max-w-2xl">Discover our handpicked selection of exceptional fragrances.</p>
          </div>
        </div>
            ...
            return (
              <div key={product.id} className="group relative flex flex-col lux-glass-card p-6">
                <div className="flex flex-col h-full">
                  ...
                  <Link href={`/products/${product.slug || product.id}`} className="block relative aspect-[4/5] overflow-hidden rounded-xl mb-6 z-10 bg-transparent shadow-2xl">
                    <Image
                      src={imageUrl}
                      alt={product.name}
                      fill
